...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      proxy: { '/api': { target: process.env.ASTRO_BACKEND_URL || 'http://127.0.0.1:8000', changeOrigin: true } },
...
function autoBackendPlugin(): Plugin {
  ...
  return {
    ...
    configureServer(server) {
      if (process.env.ASTRO_BACKEND_URL) return
      const checkAndStart = async () => {
        if (closed || checking) return
        checking = true
        try {
          const response = await fetch('http://127.0.0.1:8000/api/health', { signal: AbortSignal.timeout(2500) })
          const health = await response.json()
          if (!response.ok || health.status !== 'ok' || !health.engine?.startsWith('antariksha-')) {
            server.config.logger.error('[backend] Port 8000 is occupied by an unhealthy or different service. Check that process before restarting.')
          } else {
            attempts = 0
            nextStartAt = 0
          }
        } catch {
          if (closed || backendProc || Date.now() < nextStartAt) return
          attempts++
          nextStartAt = Date.now() + Math.min(30_000, attempts * 5_000)
          console.log('[backend] Starting astrology service on http://127.0.0.1:8000...')
          const pyPath = process.platform === 'win32'
            ? path.resolve(__dirname, '.venv/Scripts/python.exe')
            : path.resolve(__dirname, '.venv/bin/python')
          if (!existsSync(pyPath)) {
            nextStartAt = Date.now() + 30_000
            server.config.logger.error('[backend] Python environment missing. Follow backend/README.md to install dependencies.')
            return
          }
          backendProc = spawn(pyPath, ['-m', 'uvicorn', 'backend.app:app', '--host', '127.0.0.1', '--port', '8000'], {
            cwd: __dirname,
            stdio: 'inherit',
            shell: false,
            windowsHide: true,
          ...
          backendProc.on('error', (err) => {
            console.error('[backend] Failed to start backend service:', err)
          })
          backendProc.on('exit', (code) => {
            backendProc = null
            if (!closed) server.config.logger.warn(`[backend] Service exited (${code}); retrying automatically.`)
          })
        } finally {
          checking = false
        }
      ...
      void checkAndStart()
      monitor = setInterval(() => void checkAndStart(), 5000)
      // Restart only our own process when server code or local configuration changes.
      let restartTimer: ReturnType<typeof setTimeout> | undefined
      const watcher = watch(path.resolve(__dirname, 'backend'), (_event, filename) => {
        if (!filename || !(filename.endsWith('.py') || filename === '.env')) return
        clearTimeout(restartTimer)
        restartTimer = setTimeout(() => {
          if (!closed && backendProc) {
            attempts = 0
            nextStartAt = 0
            backendProc.kill()
          }
